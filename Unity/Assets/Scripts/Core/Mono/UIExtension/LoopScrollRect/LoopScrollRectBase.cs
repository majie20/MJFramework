...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [HideInInspector]
        [NonSerialized]
        public Func<int, GameObject> GetObject = null;
        ...
        [HideInInspector]
        [NonSerialized]
        public Action<Transform> ReturnObject = null;
        ...
        [HideInInspector]
        [NonSerialized]
        public Func<int, Vector2> GetItemsSize = null;
        ...
        public void ScrollToCellWithinTime(int index, float time)
        {
            ...
            {
                if (GetItemsSize != null)
                {
                    dist = GetDimension(GetItemsSize(currentFirst) - GetItemsSize(index)) + contentSpacing * (CurrentLine - TargetLine - 1);
                    dist += offset;
                }
                else
                {
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < m_Content.childCount; i++)
                {
                    ...
                    {
                        ReturnObject?.Invoke(m_Content.GetChild(i));
                        i--;
                    }
                }
                UpdateBounds(true);
        ...
        public void GetHorizonalOffsetAndSize(out float totalSize, out float offset)
        {
            if (GetItemsSize != null)
            {
                totalSize = GetItemsSize(TotalLines).x + contentSpacing * (TotalLines - 1);
                offset = m_ContentBounds.min.x - GetItemsSize(StartLine).x - contentSpacing * StartLine;
            }
            else
            {
        ...
        public void GetVerticalOffsetAndSize(out float totalSize, out float offset)
        {
            if (GetItemsSize != null)
            {
                totalSize = GetItemsSize(TotalLines).y + contentSpacing * (TotalLines - 1);
                offset = m_ContentBounds.max.y + GetItemsSize(StartLine).y + contentSpacing * StartLine;
            }
            else
            {
