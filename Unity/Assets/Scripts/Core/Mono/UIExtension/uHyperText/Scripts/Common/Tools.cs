...
namespace WXB
{
    ...
    static public partial class Tools
    {
        ...
        private static readonly Dictionary<string, string> s_fontPathDic = new Dictionary<string, string>() { { "ark_pixel_zh_cn", Model.ConstData.ARK_PIXEL_ZH_CN } };

        private static readonly Dictionary<string, string> s_spritePathDic = new Dictionary<string, string>()
        {
            { "i500", Model.ConstData.I500 }, { "brick_background", Model.ConstData.BRICK_BACKGROUND }, { "anim_001", Model.ConstData.ANIM_001 },
        };
        ...
        {
#if UNITY_EDITOR
            Font f = Application.isPlaying
                ? Model.Game.Instance.Scene.GetComponent<Model.AssetsComponent>().LoadSync<Font>(s_fontPathDic[name])
        ...
        {
#if UNITY_EDITOR
            var sprite = Application.isPlaying
                ? new DSprite(Model.Game.Instance.Scene.GetComponent<Model.SpriteComponent>().GetSprite(s_spritePathDic[name]), name)
                : new DSprite(UnityEditor.AssetDatabase.LoadAssetAtPath<Sprite>(s_spritePathDic[name]), name);
#else
            var sprite = new DSprite(Model.Game.Instance.Scene.GetComponent<Model.SpriteComponent>().GetSprite(s_spritePathDic[name]), name);
#endif
        ...
        static public Cartoon GetCartoon(string name, float space, float delay, int row, int col, bool isSingle)
        {
            Cartoon cartoon;
            ...
            if (isSingle)
            {
                ...
                cartoon = new OCartoon()
                {
                    ...
                    width = 30,
                    height = 30,
                    frame = new Cartoon.Frame() { delay = delay, sprite = sprite }
                };
            }
            else
            {
                var frames = new Cartoon.Frame[row * col];

                for (int i = 0; i < frames.Length; i++)
                {
#if UNITY_EDITOR
                    var sprite = Application.isPlaying
                        ? new DSprite(Model.Game.Instance.Scene.GetComponent<Model.SpriteComponent>().GetSprite(string.Format(s_spritePathDic[name], i + 1)), name)
                        : new DSprite(UnityEditor.AssetDatabase.LoadAssetAtPath<Sprite>(string.Format(s_spritePathDic[name], i + 1)), name);
#else
                    var sprite = new DSprite(Model.Game.Instance.Scene.GetComponent<Model.SpriteComponent>().GetSprite(string.Format(s_spritePathDic[name], i + 1)), name);
#endif
                    frames[i] = new Cartoon.Frame() { delay = delay, sprite = sprite };
                }
