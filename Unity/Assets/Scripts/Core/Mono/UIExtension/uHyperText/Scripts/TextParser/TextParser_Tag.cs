...
namespace WXB
{
    public partial class TextParser
    {
        ...
        void RegTag()
        {
            ...
            {
                string name = att.getValueAsString("n");
                float space = att.getValueAsFloat("sp", 2f);
                float delay = att.getValueAsFloat("de", 0.1f);
                int row;
                int col;
                bool isSingle = !att.exists("len");
                if (isSingle)
                {
                    row = att.getValueAsInteger("row", 1);
                    col = att.getValueAsInteger("col", 1);
                }
                else
                {
                    row = 1;
                    col = att.getValueAsInteger("len", 1);
                }
                Cartoon c = Tools.GetCartoon(name, space, delay, row, col, isSingle);
                if (c == null)
                    return;
