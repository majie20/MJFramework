...
namespace WXB
{
    ...
    public partial class SymbolText : Text, Owner, IPointerExitHandler, IPointerClickHandler, IPointerMoveHandler
    {
        [Serializable]
        ...
        [SerializeField]
        private OnClickEvent m_onClick = new OnClickEvent(); // 点击了此结点
        ...
        public TextParser Parser { get { return sTextParser; } }
        ...
        {
            get { return horizontalOverflow; }
        }

        public VerticalWrapMode VerticalOverflow
        {
            get { return verticalOverflow; }
        }

        public TextParser.Config CreateConfig()
        {
        ...
        {
            if (!isEnableMouseMove || !Tools.ScreenPointToWorldPointInRectangle(this.rectTransform, eventData.position, this.canvas.worldCamera, out var localPosition))
                return;

            this.renderCache.MouseLevel();
        }

        public void OnPointerClick(PointerEventData eventData)
        {
            if (!Tools.ScreenPointToWorldPointInRectangle(this.rectTransform, eventData.position, this.canvas.worldCamera, out var localPosition))
                return;

            var baseData = this.renderCache.MouseClick(localPosition);

            if (baseData != null)
            {
                onClick.Invoke(baseData.node);
            }
        }
        ...
        public void OnPointerMove(PointerEventData eventData)
        {
            ...
            this.renderCache.MouseEnter(localPosition);
        }
    }
}
