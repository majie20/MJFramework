        ...
        public class TextData : BaseData
        {
            ...
            public override void Render(VertexHelper vh, Rect area, Vector2 offset, float pixelsPerUnit)
            {
                ...
                if (node.d_bDynUnderline || node.d_bUnderline)
                {
                    ...
                    Vector2 startpos = new Vector2(leftBottomPos.x, leftBottomPos.y - line_height + line.minY * unitsPerPixel / 2);
                    ...
                    if (node.d_bDynUnderline)
                    {
                        ...
                        draw.AddLine(node, startpos, rect.width, Node.d_fontSize / 10f, currentColor, s_Info.uv.center, node.d_dynSpeed);
                    }
                    else
                    {
                        Tools.AddLine(vh, startpos, s_Info.uv.center, rect.width, Node.d_fontSize / 10f, currentColor);
                    }
                }
