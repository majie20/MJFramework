    ...
    public partial class RenderCache
    {
        ...
        struct Key
        {
            ...
            public Draw Get(Owner owner, Texture texture)
            {
                ...
                var dynSpeed = nodes[0].node.d_dynSpeed;
                key += texture.GetInstanceID();
                Draw draw = owner.GetDraw(drawType, key,
                    (Draw d, object p) =>
                    {
                        d.texture = texture;
                        if (d is OffsetDraw od)
                        {
                            od.Set((Rect)p);
                        }
                        else if (d is AlphaDraw alphaDraw)
                        {
                            alphaDraw.Set(dynSpeed);
                        }
                    }, offsetRect);
        ...
        public void MouseLevel()
        {
            for (int i = 0; i < DataList.Count; ++i)
            {
                DataList[i].OnMouseLevel();
            }
        }

        public BaseData MouseClick(Vector2 pos)
        {
            BaseData bd = null;
            pos -= DrawOffset;
            for (int i = 0; i < DataList.Count; ++i)
