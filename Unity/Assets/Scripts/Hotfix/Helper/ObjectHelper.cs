...
namespace Hotfix
{
    public class ObjectHelper
    {
        public static Model.Component CreateComponent(Type type, Model.Entity entity, bool isFromPool = true)
        {
            ...
            if (isFromPool)
            {
                component = Model.Game.Instance.ObjectPool.HatchComponent(type);
            }
            else
            {
                component = (Model.Component)Activator.CreateInstance(type);
            }
        ...
        public static Model.Entity CreatEntity(Model.Entity eParent, Transform parent = null, string sign = "OrdinaryGameObject", bool isFromAB = false)
        {
            Model.Entity entity = Model.Game.Instance.ObjectPool.HatchEntity();
            entity.GameObject = Model.Game.Instance.ObjectPool.HatchGameObjByName(sign, parent == null ? eParent.Transform : parent, isFromAB);
            entity.Sign = sign;
