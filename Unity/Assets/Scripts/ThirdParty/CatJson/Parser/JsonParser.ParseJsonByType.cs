...
namespace CatJson
{
    public static partial class JsonParser
    {
        ...
        public static object ParseJsonObjectByType(Type realType)
        {
            ...
            {
                object _obj = userdata1;
                Type _realType = (Type) userdata2;
                
                if (propertyInfoDict[_realType].TryGetValue(key, out PropertyInfo pi))
                {
                    //先尝试获取名为key的属性信息
                    object value = ParseJsonValueByType(nextTokenType,pi.PropertyType);
                    pi.SetValue(_obj, value);
                }
                else if (fieldInfoDict[_realType].TryGetValue(key, out FieldInfo fi))
                {
                    //属性没有 再试试字段
        ...
        private static object ParseJsonArrayByType(Type arrayType,Type elementType)
        {
            ...
            {
                IList _list = (IList) userdata1;
                Type _elementType = (Type) userdata2;
                object value = ParseJsonValueByType(nextTokenType, _elementType);
                _list.Add(value);
            });
        ...
        private static object ParseJsonObjectByDict(Type dictType, Type valueType)
        {
            ...
            ParseJsonObjectProcedure(dict, valueType,keyType == typeof(int), (userdata1, userdata2,isIntKey, key, nextTokenType) => {
               IDictionary _dict = (IDictionary)userdata1;
               Type _valueType = (Type) userdata2;
                object value = ParseJsonValueByType(nextTokenType, _valueType);
                if (!isIntKey)
                {
                ...
                {
                    //处理字典key为int的情况
                    _dict.Add(int.Parse(key.ToString()), value);
                }
