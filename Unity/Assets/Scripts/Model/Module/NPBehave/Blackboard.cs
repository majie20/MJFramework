...
namespace NPBehave
{
    public class Blackboard : IDisposable
    {
        ...
        public void Init(Blackboard parent, Clock clock)
        {
            this.clock = clock;
        ...
        private void NotifiyObservers()
        {
            ...
            for (int i = notificationsDispatch.Count - 1; i >= 0; i--)
            {
                var notification = notificationsDispatch[i];
                ...
                for (int j = observers.Count - 1; j >= 0; j--)
                {
            ...
            {
                var list = removeObservers[key];

                for (int j = list.Count - 1; j >= 0; j--)
                {
                    GetObserverList(this.observers, key).Remove(list[j]);
                }
            }
