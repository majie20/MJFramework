...
public class Reporter : MonoBehaviour
{
	...
	void Awake()
	{
        //MTODO: majie：改动了,加了radioVersion
        if (Application.platform == RuntimePlatform.Android)
        {
            AndroidJavaClass buildClass = new AndroidJavaClass("android.os.Build");
            radioVersion = buildClass.CallStatic<string>("getRadioVersion");
        }
        else
        {
            radioVersion = string.Empty;
        }
		if (!Initialized)
			Initialize();
	...
	bool isGestureDone()
	{
		if (Application.platform is RuntimePlatform.Android or RuntimePlatform.IPhonePlayer or RuntimePlatform.WebGLPlayer&&!Application.isEditor)
		{
			if (Touchscreen.current.touches.Count == 0) {
				...
				//MTODO: majie：改动了
				if ((string.IsNullOrEmpty(radioVersion) && Touchscreen.current.touches.Count == 1) || Touchscreen.current.touches.Count >= 3)
				{
					if (Touchscreen.current.touches[0].phase.value == UnityEngine.InputSystem.TouchPhase.Canceled || Touchscreen.current.touches[0].phase.value == UnityEngine.InputSystem.TouchPhase.Ended)
					    gestureDetector.Clear();
				    else if (Touchscreen.current.touches[0].phase.value == UnityEngine.InputSystem.TouchPhase.Moved) {
		...
		if (Application.platform == RuntimePlatform.Android ||
		   Application.platform == RuntimePlatform.IPhonePlayer) {
			if (Touchscreen.current.touches.Count != 1) {
				...
				if (Touchscreen.current.touches[0].phase.value == UnityEngine.InputSystem.TouchPhase.Began) {
					if (lastClickTime == -1)
	...
	Vector2 getDownPos()
	{
        //MTODO: majie：改动了,输入系统改动
        if (Application.platform == RuntimePlatform.Android ||
            Application.platform == RuntimePlatform.IPhonePlayer) {
            if (Touchscreen.current.touches.Count == 1 && Touchscreen.current.touches[0].phase.value == UnityEngine.InputSystem.TouchPhase.Began) {
